...
df2021[['Minutes','Seconds']] = df2021['TOI'].str.split(':', expand = True)
df2021['Actual GP'] = ''
df2021['Minutes'] = pd.to_numeric(df2021['Minutes'])
df2021['Seconds'] = pd.to_numeric(df2021['Seconds'])

for i in range(0, len(df2021)):
    df2021['TOI'][i] = df2021['Minutes'][i] + (df2021['Seconds'][i]/60)
    df2021['Actual GP'][i] = df2021['TOI'][i]/60

## Remove commas from SA & Svs
for i in range(0, len(df2021)):
    if ',' in df2021['SA'][i]:
        df2021[['SA1', 'SA2']] = df2021['SA'].str.split(',', expand = True)
        df2021['SA'][i] = df2021['SA1'][i] + df2021['SA2'][i]
        df2021 = df2021.drop(columns = ['SA1', 'SA2'])

    if ',' in df2021['Svs'][i]:
        df2021[['Svs1', 'Svs2']] = df2021['Svs'].str.split(',', expand = True)
        df2021['Svs'][i] = df2021['Svs1'][i] + df2021['Svs2'][i]
        df2021 = df2021.drop(columns = ['Svs1', 'Svs2'])
